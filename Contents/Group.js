...
import { Container, Header, Left, Body, Right, Button, Icon, Title, Thumbnail, Footer, Item, Input, Content, Fab, View, ListItem, Badge, Switch, Card, CardItem } from 'native-base';
import { StatusBar, Text, TextInput,Image, Keyboard, Modal,Pressable, FlatList, ScrollView } from 'react-native';
...
export default function Group({navigation,route}) {
	...
	const [selected,setSelected] = useState([]);
		...
		const remove = (name) => {
			setSelected(selected.filter((item) => item.name != name).map(({id, name}) => ({id, name})))
		}
		return (
			<Container >
				<Header style={styles.headerBackgroundColor} button>
					...
					<Body>
						<ListItem noBorder button onPress={()=>{navigation.navigate('Chat')}}>
							<View>
					...
					<Right>
						<Button transparent>
							<Icon name='search' type="MaterialIcons" style={{fontSize: 28,color:"black"}} onPress={()=>{setModalVisible(!modalVisible);}}/>
							<View style={styles.centeredView}>
								<Modal
									...
									<View style={{backgroundColor:"#FFFF"}}>
										<Item>
											<Icon name='arrow-back' type="MaterialIcons" style={{fontSize: 28,color:"black"}} onPress={()=>{setModalVisible(!modalVisible)}}/>
				...
				<View>
					<ScrollView horizontal={true}>
						{selected.map((m)=>{
							return(
								<ListItem  key={m.id} noLeft noBorder >
									<Body style={{width:60}}>
										<Button transparent onPress={()=>{remove(m.name)}}>
											<Thumbnail
										...
										<Text style={styles.removeText} numberOfLines={1}>{m.name}</Text>
									</Body>
								</ListItem>
							)
						})}
				...
				<Container>
					<FlatList
						...
						<View style={styles.listcontainer}>
							<ListItem noBorder button onPress={() =>{setSelected([...selected,{name:item.name,id:item.id}])}}>
								<Thumbnail
